...
| `sonnet-review` | `google/gemini-2.5-pro` | medium | `agent:sonnet-review:main` | specialist | Active OpenClaw strong-review fallback for `mid`/`lead` output. For Anthropic-class review, prefer direct Claude Code CLI (`claude -p --model sonnet --effort <level>`) while Anthropic API credits are unavailable. |
...
- `sonnet-review` is the OpenClaw fallback strong-review lane for `mid`/`lead` output before Hermes/Janus final checkpoint. For Anthropic-class review while API credits are unavailable, prefer direct Claude Code CLI on this VPS: `claude -p --model sonnet --effort <level> --output-format json ...`. Keep `sonnet-review`/Gemini Pro available as fallback when the CLI is not appropriate.
...
3. **Claude Code CLI / `sonnet-review` strong-review** — for `mid` and `lead` output, prefer direct Claude Code CLI on this VPS when Anthropic-class review is desired: `claude -p --model sonnet --effort <low|medium|high|xhigh|max> --output-format json`. Use OpenClaw `sonnet-review`/Gemini Pro as the fallback when the CLI is not appropriate.
