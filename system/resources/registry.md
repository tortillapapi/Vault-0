...
| Hermes profiles (all six) | `/root/.hermes/profiles/` + the default profile | Roster, models, and what drives each: `system/configs/hermes-profiles.md`. Note `hermesparser` runs a **daily production** audit despite its own description claiming no production role |
...
| nightly OpenClaw rotation | Hermes cron `572c4dc18aed`, active — verify via `hermes cron list`, **not** `openclaw cron list` (these are Hermes-scheduled jobs; the earlier "not found" note checked the wrong CLI and returned an empty OpenClaw cron list correctly, then hedged appropriately) | Full-roster session rotation (Spec 227 B4): script `/root/.hermes/scripts/openclaw-nightly-rotation.py`; invoked hourly, performs work only 04:00–04:09 America/Los_Angeles; scope `mid`, `lead`, `grunt-eng`, `grunt`, `re-review`, `email-parser`; drives `/root/bin/openclaw-grunt-session-maintenance.py` with explicit `--allow-agent` (tool defaults to `grunt`/`grunt-eng`); guards: active-task check, fresh `.progress` collision check, live-roster drift fail-closed, `safe_to_rotate` preflight, post-check. **B4: PROVEN.** First run (2026-08-11) false-alarmed `skipped` — traced to `maintenance_cmd()` unconditionally passing `--force` into the post-rotation `--dry-run` verification call, making `would_rotate` trivially true regardless of real state; rotation itself had actually succeeded (session files verified empty at the time). Fixed by adding a `force` parameter, `False` on that one call site. Second run (2026-08-12) completed clean: `state.status=rotated`, all 6 `sessions.json` verified `{}`, usage caches reset, zero problems. |
