...
- Fix: added `PrivateTmp=yes` and `ReadWritePaths=/root/finance-data/db`, `/root/.openclaw`, `/root/.hermes/google-accounts/{mramirez021111,themetalman13}` to `/etc/systemd/system/notion-sync.service` (dated `.bak` taken first, per standing rule). `systemctl daemon-reload` + verified live: forced brief push now shows real Orders/Money/Today data and no roster warning on the actual START SCREEN page.
- Separately bumped `COMMAND_TIMEOUT` 8→25s in `/root/mission-control/app/collectors.py` (`get_agents()`, shared by `notion_sync`): `openclaw agents list --bindings --json` measured 5.5-6s under normal load, leaving almost no margin against an 8s timeout. Mission Control service restarted to pick it up. This file also has unrelated pre-existing uncommitted changes (`usage.py`, `README.md`, `tests/test_usage.py`) from other in-flight work — left untouched, not committed.

## [2026-08-01T09:50:00Z] ops | [cc] Spec 217 — Inventory Command Center off-box backup + vault documentation
- **Phase 1 (local commit) COMPLETE:** removed the 0-byte tracked root `bridge.db` decoy, added `/bridge.db` to `.gitignore`; committed the inventory (Specs 185/188/207), prep-center (205), TCG (192/193/187/184), and schema/workbook (184/194/195/209/210) work in 5 logical commits on `master`. Working tree clean; 0 tracked files under `db/` or `reports/`; real `db/bridge.db` untouched (0600, ~7.9 MB). Pre-217 DB backup in `db/backups/`.
- **Phase 2 (private remote) BLOCKED:** `git remote add origin git@github.com:tortillapapi/command-center.git` configured; push failed "Repository not found" — SSH auth verified (`Hi tortillapapi!`), so the empty PRIVATE repo must be created by Papi. Wrote `/root/tasks/217-command-center-offbox-backup.blocked`; `.done` will report PARTIAL. One-liner to finish: `git -C /root/command-center push -u origin master`.
- **Phase 3 (vault) COMPLETE:** created `system/projects/inventory-command-center.md` — what the system is, relationship to [[profit-engine]] (snapshots via `extract_snapshots.py`), explicit disambiguation from the retired Inventory Tracker, correct paths (real DB `db/bridge.db` 0600; no usable root DB), real live DB row counts (27 tables incl. `inventory_pipeline` 3,042; `purchase_orders` 95; `tcg_items` 138; `prep_center_items` 17), Spec trail 184–211 (no Spec 212 on disk), operational caveats (TCG refresh broken → Spec 216; sales.db stale since 2026-07-08 → Spec 215). Aggregate-only. Minimally updated `system/projects/index.md`, `system/projects/profit-engine.md` (successor pointer), `system/resources/registry.md` (`/root/command-center` + remote).
...
## [2026-07-30T07:12:08Z] review | [hermes] Spec 205 complete — drift validated and workbook published
- Accepted exact reconciliation: `$50,472.71 + $1,350.00 prep + $7,571.04 TCG - $50.00 synthetic FBA = $59,343.75`.
- Verified run 8 has no active TCG duplicates, no historical aggregation, no price-semantics inflation, and no synthetic FBA inventory.
...
## [2026-08-01T10:44:06Z] ops | [cc] Specs 213-218 — VPS efficiency remediation (items 1-6 of Papi's audit)
- **213** Parser audit alerting repaired. The Hermes runner + audit prompt alerted via `hermes send`, but Hermes has no Telegram channel — every anomaly alert since the Spec 202 migration was silently dropped (2026-07-30 and 07-31 were both ANOMALIES). All 5 call sites now use `openclaw message send`; suppression removed so delivery failures are visible in the journal. Live delivery verified.
- **214** Daily LLM audit lanes collapsed 3 -> 1 per Papi's decision. `hermes-parser-audit` retained; `parser-cc-review` (Opus) and `parser-codex-review` (GPT-5.5) masked reversibly (unit files, scripts and isolated config dirs untouched; DECOMMISSIONED.md written to both). New deterministic `parser-healthcheck.timer` at 09:35 PT with an opt-in (`ESCALATE=1`, currently off) escalation hook. Also detects the stale duplicate `## 2026-07-25` header.
- **215** Profit Engine scheduled refresh built. `sales.db` had not synced since 2026-07-08 (Amazon orders cursor 2026-06-29) while `finance.db` was current — reconciliations compared fresh spend to 24-day-old revenue. `scheduled_refresh.py` (dry-run default, backup-before-write, 429 backoff, cursor-safe) staged but **`profit-refresh.timer` deliberately NOT enabled** — pending Papi's approval. Read-only `profit-freshness-check.timer` IS enabled and alerts >72h staleness. 308 tests pass.
- **216** TCG price refresh diagnosed: NOT a code defect. PPT API key is server-side blocked — `403 "API key blocked for abuse" (Block #3: exceeded 50 429s in 5 min, free plan)`, retryAfter ~5 days. The uncommitted `limit=5` sealed-fetch diff was exonerated (Jul 29 priced 17 with it live). Exit-code contract added: 0-priced or >50% failure now sets `status='failed'` and exits non-zero; `OnFailure=watchdog-alert@` wired. **Open:** `DELAY=0.3` pacing is what triggers the blocks — needs raising plus 429 backoff before pricing resumes.
- **217** `/root/command-center` committed (11 dirty files incl. untracked `scripts/inventory/` and `scripts/prep_center/` trees) and remote configured. Push **blocked** — `github.com/tortillapapi/command-center` does not exist yet; Papi must create it empty+private, then `git -C /root/command-center push -u origin master`. 0-byte tracked `bridge.db` decoy at the repo root removed (real DB is `db/bridge.db`, 0600). New vault page `system/projects/inventory-command-center` closes a ~3-week documentation gap covering specs 184-211.
- **218** Watchdog now monitors its own alerting layer. `watchdog-alert@mission-control-usage` had been `failed` for 9 days unseen. Added glob-based `watchdog-alert@*` coverage + a delivery-path self-check that forces non-zero exit when Telegram is unreachable; removed dead `nightly-backup` checks (Spec 126 cancelled) and retired-lane references; added `hermes-parser-audit` and `tcg-price-refresh` to the sweep.
- **Fleet note:** OpenClaw/Hermes DeepSeek V4 Flash lanes (`grunt`, `grunt-eng`, `hermesbuild`) are **down** — `403: model requires explicit workspace opt-in`. The OpenCode subscription itself is healthy (35% monthly). All grunt-tier work above ran on GLM 5.2 instead. `operating-rules.md` still states grunt/grunt-eng run DeepSeek by default; correct once the opt-in is granted.

## [2026-08-01T19:42:40Z] ops | [hermes] DeepSeek V4 Flash opt-in enabled and affected grunt lanes restored
- Papi enabled OpenCode Go's China-hosted-model opt-in after the prior 403 region gate. This is an explicit data-residency choice and must not be silently enabled for another workspace or profile.
- Updated `/root/.hermes/profiles/milo/config.yaml` so Milo's saved default is `opencode-go/deepseek-v4-flash`; gracefully reloaded `hermes-gateway-milo.service`.
- Updated `/root/.openclaw/openclaw.json` so `grunt` and `grunt-eng` saved defaults and allowed-model maps are `opencode-go/deepseek-v4-flash`. `mid`, `lead`, `re-review`, and `email-parser` were unchanged.
- Verification passed: explicit probes for Milo, `grunt`, and `grunt-eng`; saved-default probes for all three; returned provider/model metadata `opencode-go`/`deepseek-v4-flash`; OpenClaw config validation; Milo gateway active with Telegram HTTPS connections established.
- Rollback copies: `/root/.openclaw/openclaw.json.bak-hermes-20260801T193549Z` and `/root/.hermes/profiles/milo/config.yaml.bak-hermes-20260801T193549Z`.

## [2026-08-01T20:52:13Z] docs | [cc] Spec 220 — vault doc correction pass (core-index + resource registry)
- `core-index.md` Active Services: n8n Order Parser bullet corrected — parser runtime is `order-parser.timer` (systemd, 09:00 PT / 16:00 UTC); n8n workflows retired-but-retained; daily accuracy audit is `hermes-parser-audit.timer` at 09:20 PT / 16:20 UTC, ongoing (no end date); added `parser-healthcheck.timer` (09:35 PT / 16:35 UTC, deterministic, no LLM — Spec 214). Pointer to `system/logs/n8n-parser-daily-check.md` kept.
- `system/resources/registry.md` scheduled-jobs rows: OC cron `parser-daily-audit` (id `b769b0b5…`) marked RETIRED/DISABLED (`enabled: false`, verified live in openclaw.sqlite; superseded by `hermes-parser-audit.timer`, Spec 202); `parser-cc-review.timer` + `parser-codex-review.timer` marked MASKED as of 2026-08-01 (Spec 214, reversible via `systemctl unmask`); added `parser-healthcheck.timer` (enabled), `profit-freshness-check.timer` (enabled, read-only staleness alarm), `profit-refresh.timer` (installed but deliberately DISABLED, Spec 215). `last_synced` → 2026-08-01.
- All timer/enabled/masked claims verified against live systemd state before writing.

## [2026-08-01T21:00:27Z] ops | [cc] Spec 222 — orchestration workspace hygiene (archive + stale progress closeout)
- Moved 12 stale `.progress` markers (specs 197-201, 2026-07-29/30, each verified against a matching `.done`) to `/root/tasks/archive/hygiene-20260801T205949Z-spec197-201-progress/` — live `/root/tasks` now has **0** `.progress` files, so the OpenClaw grunt-session watchdog rotation gate is unblocked.
- Archived 218 task artifacts (`.txt`, `.done`, `.oc-stdout.json`, `.oc-stderr.log`, `.log`, `.oc-exit`, `.review`) for the 68 specs ≤ 212 with terminal frontmatter status into `/root/tasks/archive/spec122-211/` (move-only, MANIFEST.txt written; one documented exception: `199-phase1-canonical-closeout` siblings, whose `.progress` Part A named).
- `/root/tasks` file count 1202 → 972. `/root/specs/*.md` (150) and `/root/reviews/` (384) untouched; `hermes-parser-audit-production-prompt.txt` untouched; `hermes-parser-audit.timer` still active; no `.lock`/`.last-review` changes.
- Added `## Workspace hygiene` section to `system/workflows/peer-orchestrator-protocol.md` recording the archive rule.

## [2026-08-01T21:21:23Z] ops | [cc] Spec 223 — parser audit precision methodology fix (Item 7, Part A)
- Root cause: the daily audit selected precision candidates by `Last Updated` (refreshes on status/tracking changes and the nightly Master rebuild) but confirmed them against the step-2 email window (`newer_than:1d`). Long-tail business orders (Alibaba …0582 $7,700/140u email 2026-06-15, US Mint …3691 email 2026-02-26, Amazon.com …6259 email 2026-04-23 — all verified live in Master) looked "recent" and were branded FALSE POSITIVES. Parser was correct; audit was wrong.
- §3 now selects precision candidates by `Date of Email` (column F, inside the A:I read) within the same lookback window step 2 uses; added third outcome **NOT EVALUABLE** (email predates window) — never a FP, never against precision. §4 log format extended: `Precision: <R> in-window rows | <C> confirmed | <N> not evaluable (email outside window) | FALSE POSITIVES: ...`; verdict rule: ANOMALIES only on workflow error / recall MISS / in-window FP; NE alone → OK. §5 alert clarified (NE alone does not alert).
...
- Durable evidence: `/root/reviews/225-phase2-notion-router-production-activation.md`; completion marker `/root/tasks/archive/spec225-notion-router-phase2/225-phase2-notion-router-production-activation.done`; project page updated at `system/projects/notion-quick-capture-router.md`.
