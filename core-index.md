...
## Active Services
_Timer states verified against `systemctl` 2026-08-11._
- openclaw-gateway — **user** unit `openclaw-gateway.service`, active
...
- ~~cleanup-inventory-tracker.timer~~ — **disabled/inactive** (verified 2026-08-11). Its one-shot archival of the decommissioned inventory-tracker workspace is done; listed here only so the absence is not mistaken for a missing service. Do not re-enable — see the standing rule against reviving the Inventory Tracker in [[system/cheatsheets/operating-rules]]
